...
class DetailViewController: UIViewController,UIPickerViewDataSource,UIPickerViewDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        print([myTableViewController.currentRow])
        ...
        //指定輸入性別與班別時使用對應的滾輪來選擇資料（不使用預設鍵盤）
    ...
    @objc func keyBoardWillShow(_ sender:Notification)
    {
        ...
        if let keyBoardHeight = (sender.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size.height{
            ...
            {
                //移動y軸底緣位置與可是高度之間的差值
                self.view.frame.origin.y -= currentObjectBottonYPosition - visiableHeight + 10
            }
        }
    }
    //鍵盤收合時，由通知中心呼叫的函式
    ...
    {
        if pickerView.tag == 2//性別滾輪
        {
            return arrItem.count
        }
        else if pickerView.tag == 4 //班別滾輪
        {
    ...
    {
        if pickerView.tag == 2  //性別的滾輪
        {
            return arrItem[row]
        }
        else if pickerView.tag == 4 //班別的滾輪
        {
            return arrType[row]
        }
        return arrType[row]
